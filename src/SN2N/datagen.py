...
class generator2D:
    ...
    def execute(self, flage=1):
        ...
        num_files = len(datapath_list)

        for ll in range(num_files):
            ...
            try:
                ...
                if self.P2Pmode == 3:
                    image_data_b = tifffile.imread(
                        datapath_list[random.randint(0, num_files - 1)]
                    )
                else:
                    image_data_b = []
                ...
                for circlelarge in range(roll):
                    if times >= 1:
                        image_data_pre = self.random_interchange(
                            imga=image_data, imgb=image_data_b
                        )
                        for circle in range(times - 1):
                            image_data_pre = self.random_interchange(
    ...
    def fourier_inter(self, image_stack):
        ...
        for slice in range(t):
            ...
            imgf1[slice, :, :] = img1[
                idx[0] - 1 : idx[0] + ttttem1, idx[1] - 1 : idx[1] + ttttem2
            ]
            imgf1[imgf1 < 0] = 0
        return imgf1
    ...
    def interpft(self, x, ny, dim=0):
        ...
        b = np.concatenate(
            (a[0:nyqst, :], np.zeros(shape=(ny - m, n)), a[nyqst:m, :]), 0
        )
    ...
    def interchange_multiple(self, imga, imgb, ifdirect=False):
        ...
        h = size[0]
        w = size[1]
        if h < np.min((np.size(imga, 0), np.size(imgb, 0))) and w < np.min(
            (np.size(imga, 1), np.size(imgb, 1))
        ):
...
class generator3D:
    ...
    def execute(self, flage=1):
        ...
        for ll in range(num_files):
            ...
            for circlelarge in range(roll):
                if times >= 1:
                    image_data_pre = self.random_interchange(
                        imga=image_data_stack, imgb=image_data_b
                    )
                    for circle in range(times - 1):
                        image_data_pre = self.random_interchange(
                            imga=image_data_pre, imgb=image_data_b
        ...
        for f in range(frame):
            ...
            if self.BAmode == 1:
                ...
                for taxial in range(t - self.multi_frame):
                    ...
                    imgpart_aug[taxial, 0 : imsize[0], imsize[1] : 2 * imsize[1]] = (
                        temp_r_aug
                    )
                    imgpart_aug_copy = imgpart_aug.copy()
                    imgpart_list.append(imgpart_aug_copy)
                ...
                for m in range(1, 8):
                    for taxial in range(t - self.multi_frame):
                        ...
                        imgpart_aug[taxial, 0 : imsize[0], 0 : imsize[1]] = temp_l_aug
                        imgpart_aug[
                            taxial, 0 : imsize[0], imsize[1] : 2 * imsize[1]
                        ] = temp_r_aug
                        imgpart_aug_copy = imgpart_aug.copy()
                        imgpart_list.append(imgpart_aug_copy)
    ...
    def interchange_multiple3d(self, imga, imgb, ifdirect=False):
        ...
        h = size[0]
        w = size[1]
        if h < np.min((np.size(imga, 1), np.size(imgb, 1))) and w < np.min(
            (np.size(imga, 2), np.size(imgb, 2))
        ):
