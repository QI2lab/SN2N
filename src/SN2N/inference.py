                ...
                with torch.no_grad():
                    for root, dirs, files in os.walk(img_path):
                        for j, Ufile in enumerate(files):
                            ...
                            data = self._normalize_raw_volume(np.squeeze(input_tif))
                            model_input_image_shape = (16, 640, 640)
                            model_output_image_shape = (16, 640, 640)

                            if len(model_input_image_shape) != len(
                            ...
                            block_weight = np.ones(
                                [
                                    m - 2 * o
                                    for m, o in zip(
                                        model_output_image_shape, overlap_shape
                                    )
                                ],
                                dtype=np.float32,
                            )
                            ...
                            for image in data:
                                ...
                                blocks = list(
                                    itertools.product(
                                        *[
                                            np.arange(n) * s
                                            for n, s in zip(num_steps, step_shape)
                                        ]
                                    )
                                )
                                    ...
                                    for batch_index, tl in enumerate(
                                        ...
                                        if model_input_image_shape != m.shape:
                                            pad_width = [
                                                (0, b - s)
                                                for b, s in zip(
                                                    model_input_image_shape, m.shape
                                                )
                                            ]
