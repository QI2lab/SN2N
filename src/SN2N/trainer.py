...
class net2D:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for inputs, labels in self.load_batch2d(self.dataset_path):
                ...
                inputs, labels = (
                    inputs.to(self.device, dtype=torch.float32),
                ...
                if self.sn2n_loss != 0:
                    ...
                    loss = (loss1 + loss2 + self.sn2n_loss * loss3) / (
                        2 + self.sn2n_loss
                    )
                else:
                    loss = loss1
                ...
                print(
                    "[Epoch %d/%d] [Batch %d/%d] [loss:%f] time:%s"
    ...
    def load_batch2d(self, traindata_path):
        ...
        for i in range(batch_num):
            location = np.array(
                np.random.randint(low=0, high=len(path), size=(self.bs, 1), dtype="int")
            )
